...
#include "MyShaders/Public/MarchingCubes.h"
...
class MYSHADERS_API FMarchingCubes : public FGlobalShader
{
public:
	...
	class FMarchingCubes_Perm_TEST : SHADER_PERMUTATION_INT("TEST", 1);
	using FPermutationDomain = TShaderPermutationDomain<
		FMarchingCubes_Perm_TEST
	>;
...
void FMarchingCubesInterface::DispatchRenderThread(FRHICommandListImmediate& RHICmdList, FMarchingCubesDispatchParams Params, TFunction<void(TArray<FTriangle> OutputVal)> AsyncCallback) {
...

	{
		...
		// PermutationVector.Set<FMarchingCubes::FMyPermutationName>(12345);
		...
		if (bIsShaderValid) {
			...
			const void* RawData = (void*)Params.OctreeNodes;
			int NumInputs = 2;
			...
			GraphBuilder.AddPass(
				RDG_EVENT_NAME("ExecuteMarchingCubes"),
				PassParameters,
			...
			FRHIGPUBufferReadback* GPUBufferReadback = new FRHIGPUBufferReadback(TEXT("ExecuteMarchingCubesOutput"));
